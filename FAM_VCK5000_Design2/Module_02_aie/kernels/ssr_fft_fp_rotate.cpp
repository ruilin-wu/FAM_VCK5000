...
{
  // Compute the rotate factors based on given twiddle coefficient in the file
  ...
  *py2 = vx2;
  ...
  py1 += (64 -56)/4;// +2 -> 64
  py2 += (128-60)/4;// +17 -> 128
  v4cfloat * restrict py3 = tw+(192/4);// +48 -> 192
...
void ssr_fft_fp_rotate::run( input_buffer_1d<int,1024,0>& __restrict x0,
                             input_buffer_1d<int,1024,0>& __restrict x1,
                             output_buffer_1d<int,1024>& __restrict y0,
                             output_buffer_1d<int,1024>& __restrict y1 )
{
  fp_dds(twrom+idx);
